...
# -----------------------------------
version=$(python3 -V 2>&1 | awk '{print $2}' | cut -d '.' -f 1)
if [[ -z "$version" ]]; then
...
brew install portaudio19-dev python3-pyaudio python3-pyaudio && /
brew install libasound2-plugins libsox-fmt-all libsox-dev libxml2-dev libxslt-dev sox ffmpeg && /
brew install espeak && /
brew install libcairo2-dev libgirepository1.0-dev gir1.2-gtk-3.0  && /
